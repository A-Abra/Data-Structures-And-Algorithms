...
//     elts.add(v.element());
  ...
  protected void levelorderElements(BSTNode<K, E> v, ArrayList<E> elts) {
	  Queue<BSTNode<K,E>> q = new LinkedList<>();
	  
	  q.add(v);
	  while(!q.isEmpty()) {
		  BSTNode<K,E> current = q.remove();
		  if(current.left() != null)
			  q.add(current.left());
		  if(current.right() != null)
			  q.add(current.right());
		  elts.add(current.element());
	  }
  }
  ...
  public Iterable<E> values() {
    ...
    if (size() != 0)
      levelorderElements(root, elements); // assign positions in order
    return elements;
  }
  ...
  protected void rangehelp(BSTNode<K, E> v, ArrayList<E> elts, K a, K b) {
	  ...
	  //evals if current node falls between two keys
	  //prints inorder (alpha)
  ...
  public ArrayList<E> smallCount(K a) {
	  ArrayList<E> elements = new ArrayList<E>();
	  countfind(root, elements, a);
	  return elements;
  }
  
  protected void countfind(BSTNode<K,E> v, ArrayList<E> elts, K a) {
	  if (v.left() != null)
		  countfind(v.left(), elts, a);
