...
while(ind1+ind2 != 0) {
	boolean flag = true;
	boolean[] visit = new boolean[act.size()];
	ArrayDeque<Integer> toExplore = new ArrayDeque<Integer>();
	toExplore.add(ind1);
	...
	while(true) {
		...
		ind1=toExplore.removeFirst();
		if(ind1==ind2)
			break;
		for(Integer neighbors:g.neighbors(ind1)) {
			if(visit[neighbors])
				continue;
			g.setMark(neighbors, ind1);
			visit[neighbors]=true;
			toExplore.add(neighbors);
		}
	}
	...
	if(flag) {
		...
		while(!sol.isEmpty()) {
			...
			int act1=sol.pop();
			int act2=sol.peek();
			String actor1=act.get(act1).name;
			String actor2=act.get(act2).name;
			String commonMovie = "";
			ActorRecord person1 = act.get(act1);
			ActorRecord person2 = act.get(act2);
			for(String currMvi:person1.movies) {
				if(person2.movies.contains(currMvi)) {
					commonMovie=currMvi;
					break;
				}
			}
			System.out.println(actor1+" Appeared with "+actor2+" in "+commonMovie);
		}
	}
	else {
		String actor1=act.get(ind1).name;
		String actor2=act.get(ind2).name;
		System.out.print("No path between "+actor1+ " and "+actor2);
	}
	System.out.println();
	System.out.println("Enter source and destination indices:");
	ind1=scan.nextInt();
